/**
 * ETH Payment Status API
 * POST /api/payments/eth-status
 *
 * Triggers an immediate on-chain check for an ETH deposit and returns
...
import { checkETHPaymentStatus } from '@/lib/eth-detection'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const status = await checkETHPaymentStatus(depositId)
    ...
    return addSecurityHeaders(
      successResponse({
        ...
        ethAmount: status.amount,
      })
    )
  } catch (error: any) {
    console.error('[ETH Status] Error checking status:', error)
